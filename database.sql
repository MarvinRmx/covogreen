...
-- Procédures
--
CREATE PROCEDURE `createCar` (`_id_user` INTEGER, `_licencePlate` VARCHAR(255), `_make` VARCHAR(255), `_model` VARCHAR(255), `_capacity` INTEGER)  BEGIN
    	DECLARE lastCar INT;
...
CREATE PROCEDURE `deleteUser` (`_id_user` INTEGER)  BEGIN
    	DECLARE _id_car INT;
